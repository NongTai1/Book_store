...
import NongDucTai.com.DATotNghiep.common.ProductSearch;
...
public class ProductService {
	...
	public List<Product> search(final ProductSearch productSearch) {
		...
		if (productSearch.getTypePrice() != null && !productSearch.getTypePrice().isEmpty()) {
			...
			if (price == 1) {
				jpql += " and p.status= " + productSearch.getStatusProduct() +" and price >= " + 0 + " and price <= " + 25000;
			}
			if (price == 2) {
				jpql += " and p.status= " + productSearch.getStatusProduct() +" and price >= " + 25000 + " and price <= " + 50000;

			}
			if (price == 3) {
				jpql += " and p.status= " + productSearch.getStatusProduct() +" and price >= " + 50000 + " and price <= " + 100000;

			}
			if (price == 4) {
				jpql += " and p.status= " + productSearch.getStatusProduct() +" and price >= " + 100000 + " and price <= " + 300000;

			}
			if (price == 5) {
				jpql +=" and p.status= " + productSearch.getStatusProduct() + " and price >= " + 300000;
			}
		}
			...
			if (product.getId() != null) { // chỉnh sửa
				...
				if (!isEmptyUploadFile(images)) { // nếu admin sửa ảnh sản phẩm
					...
					for (ProductImages _images : productImages) {
						new File("D:/DATN/DATotNghiep/upload/" + _images.getPath()).delete();
					}
			...
			if (!isEmptyUploadFile(images)) { // nếu admin upload ảnh
				for (MultipartFile image : images) {
					...
					image.transferTo(new File(
							"D:/DATN/DATotNghiep/upload/" + image.getOriginalFilename()));
