...
import NongDucTai.com.DATotNghiep.GeneratePassword;
import NongDucTai.com.DATotNghiep.entities.Role;
import NongDucTai.com.DATotNghiep.entities.User;
import NongDucTai.com.DATotNghiep.repositories.RoleRepo;
import NongDucTai.com.DATotNghiep.repositories.UserRepo;
		...
		try {
			if (user.getId() != null) { // chỉnh sửa
				...
				if (!isEmptyUploadFile(images)) { // nếu admin sửa ảnh sản phẩm
					// xoá ảnh cũ đi
					 new File("D:/DATN/DATotNghiep/upload_avt/" + user.getAvatar()).delete();
			...
			if (!isEmptyUploadFile(images)) { // nếu admin upload ảnh
				for (MultipartFile image : images) {
					...
					image.transferTo(new File(
							"D:/DATN/DATotNghiep/upload_avt/" + image.getOriginalFilename()));
